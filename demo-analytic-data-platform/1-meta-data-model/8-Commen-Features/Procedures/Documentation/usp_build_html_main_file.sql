...
BEGIN
  ...
  IF (1=1 /* Build HTML-file */) BEGIN
    ...
    DELETE FROM mdm.html_file_text WHERE id_dataset = @id;

    IF (1=1 /* Build <head> part of file. */) BEGIN
      EXEC f @id, @tx;   SET @tx = N'<!DOCTYPE html>';
      EXEC f @id, @tx;   SET @tx = N'<html lang="en-US">';
      EXEC f @id, @tx;   SET @tx = N'  ';
      EXEC f @id, @tx;   SET @tx = N'  <title>Documentation of "Simple Analytic Data Platform"</title>';
      EXEC f @id, @tx;   SET @tx = N'  ';
      ...
      EXEC f @id, @tx;   SET @tx = N'    <meta name="viewport"                    content="width=device-width, initial-scale=1.0">';
      ...
      EXEC f @id, @tx;   SET @tx = N'    <style>' 
      EXEC f @id, @tx;   SET @tx = N'        .dataset {' 
      EXEC f @id, @tx;   SET @tx = N'            cursor: pointer;' 
      EXEC f @id, @tx;   SET @tx = N'            user-select: none;' 
      EXEC f @id, @tx;   SET @tx = N'        }' 
      EXEC f @id, @tx;   SET @tx = N'        .folder {' 
      EXEC f @id, @tx;   SET @tx = N'            cursor: pointer;' 
      EXEC f @id, @tx;   SET @tx = N'            user-select: none;' 
      EXEC f @id, @tx;   SET @tx = N'        }' 
      EXEC f @id, @tx;   SET @tx = N'        .nested {' 
      EXEC f @id, @tx;   SET @tx = N'            display: none;' 
      EXEC f @id, @tx;   SET @tx = N'        }' 
      EXEC f @id, @tx;   SET @tx = N'        .active {' 
      EXEC f @id, @tx;   SET @tx = N'            display: block;' 
      EXEC f @id, @tx;   SET @tx = N'        }' 
      EXEC f @id, @tx;   SET @tx = N'    </style>' 
      EXEC f @id, @tx;   SET @tx = N'    ';
      EXEC f @id, @tx;   SET @tx = N'  </head>';
    ...
    IF (1=1 /* Build HTML <body>-element */) BEGIN
      ...
      IF (1=1 /* Begin : List All Ingestion(s) BY Group */) BEGIN
        ...
        EXEC f @id, @tx;     SET @tx = N'          <li><!-- Begin: ' + @nm_part + N'(s) by Group --><span class="folder">📁 ' + @nm_part + '(s) by Group</span>';
        EXEC f @id, @tx;     SET @tx = N'            <ul class="nested"><!-- List All ' + @nm_part + '(s) Dataset By Group -->'; 
        ...
        WHILE (@ni_grp < @mx_grp) BEGIN 
          ...
          EXEC f @id, @tx;   SET @tx = N'              <li class="details" id="group/' + ISNULL(mdm.json_value(@ni_grp, @tx_grp, 'id_group'),'n/a') + N'"><span class="folder">📁 ' + ISNULL(mdm.json_value(@ni_grp, @tx_grp, 'fn_group'),'n/a') + N'</span>';
          EXEC f @id, @tx;   SET @tx = N'                <ul class="nested">'; 
          ...
          WHILE (@ni_dst < @mx_dst) BEGIN
            ...
            EXEC f @id, @tx; SET @tx = N'                  <li class="dataset" id="datasets/' + ISNULL(mdm.json_value(@ni_dst, @tx_dst, 'id_dataset'),'n/a') + N'.html">📄 ' + ISNULL(mdm.json_value(@ni_dst, @tx_dst, 'fn_dataset'),'n/a') + N'</li>'; 
            
          SET @ni_dst += 1; END
          
          EXEC f @id, @tx;   SET @tx = N'                </ul>';
          EXEC f @id, @tx;   SET @tx = N'              </li>'; 
        
        SET @ni_grp += 1; END
        
        EXEC f @id, @tx;     SET @tx = N'            </ul>';
        EXEC f @id, @tx;     SET @tx = N'         </li><!-- End: ' + @nm_part + '(s) by Group -->';
      ...
      EXEC f @id, @tx;       SET @tx = N'      </div>';
      EXEC f @id, @tx;       SET @tx = N'    </div>';
      EXEC f @id, @tx;       SET @tx = N'    <script>'
      EXEC f @id, @tx;       SET @tx = N'      document.querySelectorAll(''.folder'').forEach(folder => {'
      EXEC f @id, @tx;       SET @tx = N'        folder.addEventListener(''click'', function() {'
      EXEC f @id, @tx;       SET @tx = N'          this.parentElement.querySelector(''.nested'').classList.toggle(''active'');'
      EXEC f @id, @tx;       SET @tx = N'          this.classList.toggle(''folder-open'');'
      EXEC f @id, @tx;       SET @tx = N'        });'
      EXEC f @id, @tx;       SET @tx = N'      });'
      EXEC f @id, @tx;       SET @tx = N'      document.querySelectorAll(''.dataset'').forEach(detail => {'
      EXEC f @id, @tx;       SET @tx = N'        detail.addEventListener(''click'', function() {'
      EXEC f @id, @tx;       SET @tx = N'          const iframe = document.getElementById(''iframe-dataset'');'
      EXEC f @id, @tx;       SET @tx = N'          iframe.src = this.id;'
      EXEC f @id, @tx;       SET @tx = N'        });'
      EXEC f @id, @tx;       SET @tx = N'      });'
      EXEC f @id, @tx;       SET @tx = N'    </script>'
      EXEC f @id, @tx;       SET @tx = N'    ';
    ...
    IF (1=1 /* Build HTML </html>-element (end) */) BEGIN
      ...
      EXEC f @id, @tx;       SET @tx = N'';
      EXEC f @id, @tx;       -- Write the last line.
