DROP PROCEDURE IF EXISTS ##usp_insert_all_current_dq_datasets;
GO
CREATE PROCEDURE ##usp_insert_all_current_dq_datasets AS BEGIN
  INSERT INTO tsa_dta.tsa_dataset (id_dataset, id_development_status, id_group, is_ingestion, fn_dataset, fd_dataset, nm_target_schema, nm_target_table, tx_source_query) 
  SELECT id_dataset, id_development_status, id_group, is_ingestion, fn_dataset, fd_dataset, nm_target_schema, nm_target_table, tx_source_query
  FROM dta.dataset
  WHERE meta_is_active = 1
  AND   nm_target_schema IN ('dq_result', 'dq_totals', 'dta_dq_aggregates', 'dqm');
END
GO

DROP PROCEDURE IF EXISTS ##usp_insert_all_current_aggregate_dq_datasets;
GO
CREATE PROCEDURE ##usp_insert_all_current_aggregate_dq_datasets AS BEGIN
  INSERT INTO tsa_dta.tsa_dataset (id_dataset, id_development_status, id_group, is_ingestion, fn_dataset, fd_dataset, nm_target_schema, nm_target_table, tx_source_query) 
  SELECT id_dataset, id_development_status, id_group, is_ingestion, fn_dataset, fd_dataset, nm_target_schema, nm_target_table, tx_source_query
  FROM dta.dataset
  WHERE meta_is_active = 1
...
DROP PROCEDURE IF EXISTS ##usp_delete_all_current_aggregate_dq_datasets;
GO
CREATE PROCEDURE ##usp_delete_all_current_aggregate_dq_datasets AS BEGIN
  DELETE FROM tsa_dta.tsa_dataset
  WHERE nm_target_schema IN ('dta_dq_aggregates', 'dqm');
END
GO

DROP PROCEDURE IF EXISTS ##usp_dq_aggregates_as_datasets_for_results;
GO
CREATE PROCEDURE ##usp_dq_aggregates_as_datasets_for_results AS BEGIN DECLARE 

    @id_development_status_prd CHAR(32) = (SELECT id_development_status FROM srd.development_status WHERE meta_is_active = 1 AND cd_development_status = 'PRD'),
    @id_development_status_uat CHAR(32) = (SELECT id_development_status FROM srd.development_status WHERE meta_is_active = 1 AND cd_development_status = 'UAT'),
    @id_development_status_dev CHAR(32) = (SELECT id_development_status FROM srd.development_status WHERE meta_is_active = 1 AND cd_development_status = 'DEV'),

    @id_datatype           CHAR(32) = (SELECT id_datatype           FROM srd.datatype           WHERE meta_is_active = 1 AND cd_target_datatype = 'CHAR(32)'),
    @ni_level              INT      = 1,
    @ni_inserted           INT,
  ...
  BEGIN
    SET @tx_next_ddl_query_part = ''
      ...
      +       ' dqr.id_dataset_1_bk AS id_dataset_1_bk,' 
      +       ' dqr.id_dataset_2_bk AS id_dataset_2_bk,'
      +       ' dqr.id_dataset_3_bk AS id_dataset_3_bk,'
      +       ' dqr.id_dataset_4_bk AS id_dataset_4_bk,'
      +       ' dqr.id_dataset_5_bk AS id_dataset_5_bk,'
      +       ' dqr.id_dq_result_status AS id_dq_result_status'
      + 'FROM [dq_result].[dqr_<@id_dq_control>] AS dqr'
    ...
    IF (1=1) BEGIN PRINT('/* Extract "DQ Controls" with "Created At"-dattime. */')
      ...
      WHERE dqc.meta_is_active = 1
      GROUP BY  id_dataset, id_dq_control;
    END
    IF (1=1) BEGIN PRINT('/* Extract definitions of "DQ Result"-datasets. */');
      ...
      AND   SUBSTRING(dst.nm_target_table,1,3) = 'dqr'
    ...
    SET @tx_next_ddl_query_part = ''
      ...
      +       ' dqa.id_dataset_1_bk     AS id_dataset_1_bk,' 
      +       ' dqa.id_dataset_2_bk     AS id_dataset_2_bk,'
      +       ' dqa.id_dataset_3_bk     AS id_dataset_3_bk,'
      +       ' dqa.id_dataset_4_bk     AS id_dataset_4_bk,'
      +       ' dqa.id_dataset_5_bk     AS id_dataset_5_bk,'
      +       ' dqa.id_dq_result_status AS id_dq_result_status'
      + 'FROM [dta_dq_aggregates].[dqa_<@id_dq_control>] AS dqa'
      ...
      SELECT /* Helper Attribute for "next" iteration. */
        ...
        LOWER(CONVERT(CHAR(32), HASHBYTES('MD5', CONCAT(CONVERT(NVARCHAR(MAX), ''),
         ...
         REPLACE(@tx_next_ddl_query_part, '<@id_dq_control>', q0.id_next_dataset)) AS tx_next_ddl_query_part,
        
        /* Attributes for INSERT into tsa_dta.tsa_dataset. */
        q0.id_next_dataset            AS id_dataset,
        @id_development_status_prd    AS id_development_status,
        NULL                          AS id_group,
        0                             AS is_ingestion,
    ...
    WHILE (@ni_inserted != 0) BEGIN print('/* Add more "Aggregations" of "DQ Results" to "Definitons"-datasets. */');
      ...
      SELECT /* Helper Attribute for "next" iteration. */
        ...
        CASE /*                         AS fn_dataset, */
          WHEN @ni_inserted = 2 
          THEN 'DQ Result' 
          ELSE CONVERT(NVARCHAR(128), 'DQ Result (Agg) - Level ' + CONVERT(NVARCHAR(32), q0.ni_next_level)+ '.' + FORMAT(q0.ni_next_sub_level,'N0')) END AS fn_dataset,
        CASE /*                         AS fd_dataset, */
          WHEN @ni_inserted = 2 
          THEN 'This \"Dataset\" has an \"Aggregation\" of all \"DQ Results\" for all \"DQ Controls\".' 
          ELSE CONVERT(NVARCHAR(999), 'This will aggregate the \"DQ Results\" of `' + q0.id_dataset+'` and `' + q0.id_dataset+'` into new dataset.') END AS fd_dataset,
      ...
      IF (@ni_inserted = 1) BEGIN PRINT('/* Is "Last"-aggregation of "DQ Results". */');
        SET @ni_inserted = 0;
      END
    END
    ...
    IF (1=1) BEGIN PRINT('/* Construct "Definitions for Level 1 "Attributes" of"Unions" of the "DQ Results". */');
      INSERT INTO tsa_dta.tsa_attribute (id_attribute, id_dataset, id_datatype, fn_attribute, fd_attribute, ni_ordering, nm_target_column, is_businesskey, is_nullable) SELECT     
        id_attribute     = LOWER(CONVERT(char(32), HASHBYTES('MD5', CONCAT(CONVERT(NVARCHAR(MAX), ''), '|dqr|', dst.id_dataset, '|', col.nm_target_column, '|')), 2)),
        ...
        is_businesskey   = IIF(nm_target_column IN ('id_dq_control', 'id_dq_result_status', 'id_record_meta_ch_bk'), 1, 0),
        is_nullable      = 0
    
      FROM #dta_dataset AS dst CROSS JOIN ( SELECT v.ni_ordering, v.nm_target_column, v.fn_attribute, v.fd_attribute FROM (VALUES 
		      (1, 'id_dq_control',       'ID DQ Control',           'Reference to \"DQ Control\".'),
          (2, 'id_dataset_1_bk',     'ID Dataset (meta_ch_bk)', 'The \"Businesskey\"-value of the \"Record\" from the \"Dataset 5\" checked by the \"DQ Control\".'),
          (3, 'id_dataset_2_bk',     'ID Dataset (meta_ch_bk)', 'The \"Businesskey\"-value of the \"Record\" from the \"Dataset 5\" checked by the \"DQ Control\".'),
          (4, 'id_dataset_3_bk',     'ID Dataset (meta_ch_bk)', 'The \"Businesskey\"-value of the \"Record\" from the \"Dataset 5\" checked by the \"DQ Control\".'),
          (5, 'id_dataset_4_bk',     'ID Dataset (meta_ch_bk)', 'The \"Businesskey\"-value of the \"Record\" from the \"Dataset 5\" checked by the \"DQ Control\".'),
          (6, 'id_dataset_5_bk',     'ID Dataset (meta_ch_bk)', 'The \"Businesskey\"-value of the \"Record\" from the \"Dataset 5\" checked by the \"DQ Control\".'),
          (7, 'id_dq_result_status', 'ID DQ Result Status',     'Reference to \"DQ Result Status\".') 
...
DROP PROCEDURE IF EXISTS ##usp_dq_aggregates_as_datasets_for_totals;
GO
CREATE PROCEDURE ##usp_dq_aggregates_as_datasets_for_totals AS BEGIN DECLARE 

    @id_development_status CHAR(32) = (SELECT id_development_status FROM srd.development_status WHERE meta_is_active = 1 AND cd_development_status = 'PRD'),
    @id_datatype_ch        CHAR(32) = (SELECT id_datatype           FROM srd.datatype           WHERE meta_is_active = 1 AND cd_target_datatype    = 'CHAR(32)'),
    @id_datatype_int       CHAR(32) = (SELECT id_datatype           FROM srd.datatype           WHERE meta_is_active = 1 AND cd_target_datatype    = 'INT'),
    @ni_level              INT      = 1,
    @ni_inserted           INT,
    @tx_next_ddl_query_part NVARCHAR(MAX)
  BEGIN
