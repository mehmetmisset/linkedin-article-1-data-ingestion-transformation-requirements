DROP PROCEDURE IF EXISTS ##usp_insert_dq_controls_as_datasets;
GO

CREATE PROCEDURE ##usp_insert_dq_controls_as_datasets AS DECLARE
  ...
  @id_dq_result_status_oke NVARCHAR(34) = '"' + gnc_commen.id_dq_result_status('OKE') + '"',
  @id_dq_result_status_nok NVARCHAR(34) = '"' + gnc_commen.id_dq_result_status('NOK') + '"',
  @id_dq_result_status_oos NVARCHAR(34) = '"' + gnc_commen.id_dq_result_status('OOS') + '"',
...
BEGIN
  ...
  DROP TABLE IF EXISTS #dqc; SELECT id_dq_control AS id INTO #dqc FROM tsa_dqm.tsa_dq_control WHERE LEN(ISNULL(tx_dq_control_query,'')) > 10;
  WHILE ((SELECT COUNT(*) FROM #dqc) > 0) BEGIN
    
    /* Fetch next "record". */
    SELECT @id = id FROM (SELECT TOP 1 id FROM #dqc) AS rec; DELETE FROM #dqc WHERE id = @id;
    
    IF (1=1 /* Generate "Dataset"- and "Attribute"- definitions for the "DQ Control"-results. */) BEGIN
  
      /* Extract "DQ Control". */
      SELECT @id_dq_control         = LOWER(CONVERT(CHAR(32), HASHBYTES('MD5', CONCAT(CONVERT(NVARCHAR(MAX), ''), '|', id_dq_control, '|', 'result', '|')), 2)),
      ...
      FROM tsa_dqm.tsa_dq_control WHERE id_dq_control = @id;
      ...
      INSERT INTO tsa_dta.tsa_dataset (id_dataset, id_development_status, id_group, is_ingestion, fn_dataset, fd_dataset, nm_target_schema, nm_target_table, tx_source_query) SELECT 
