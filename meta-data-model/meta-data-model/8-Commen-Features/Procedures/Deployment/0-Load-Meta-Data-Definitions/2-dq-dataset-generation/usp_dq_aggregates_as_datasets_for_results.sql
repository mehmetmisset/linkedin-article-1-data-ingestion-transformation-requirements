CREATE PROCEDURE [deployment].[usp_dq_aggregates_as_datasets_for_results] AS BEGIN 

DECLARE 
    @id_model                  CHAR(32) = (SELECT id_model              FROM mdm.current_model);

DECLARE 
    @id_development_status_prd CHAR(32) = (SELECT id_development_status FROM srd.development_status WHERE meta_is_active = 1 AND cd_development_status = 'PRD' AND id_model = @id_model),
    @id_development_status_uat CHAR(32) = (SELECT id_development_status FROM srd.development_status WHERE meta_is_active = 1 AND cd_development_status = 'UAT' AND id_model = @id_model),
    @id_development_status_dev CHAR(32) = (SELECT id_development_status FROM srd.development_status WHERE meta_is_active = 1 AND cd_development_status = 'DEV' AND id_model = @id_model),
    @id_datatype               CHAR(32) = (SELECT id_datatype           FROM srd.datatype           WHERE meta_is_active = 1 AND cd_target_datatype = 'CHAR(32)' AND id_model = @id_model),
    @ni_level              INT      = 1,
    @ni_inserted           INT,
